...
def PPV(mylist):
    tp, fn, fp = mylist[0], mylist[1], mylist[2]
...
def NPV(mylist):
    fn, fp, tn = mylist[1], mylist[2], mylist[3]
    # for the case: there is no non-VA segs for the patient,
    # then npv should be 1
    if tn + fp == 0:
        npv = 1
    # for the case: there is some VA segs, but the predictions are wrong
...
def Specificity(mylist):
    fp, tn = mylist[2], mylist[3]
    # for the case: there is no non-VA segs for the patient,
    # then spe should be 1
    if tn + fp == 0:
        specificity = 1
    else:
...
class IEGM_DataSET():
    def __init__(self, root_dir, indice_dir, mode, size, transform=None):
        ...
        csvdata_all = loadCSV(os.path.join(self.indice_dir,
                                           mode + '_indice.csv'))
    ...
    def __getitem__(self, idx):
        ...
        sample = {'IEGM_seg': IEGM_seg.squeeze(-1), 'label': label}
